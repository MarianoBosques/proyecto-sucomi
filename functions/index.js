...
// Opciones por defecto para funciones (compatibilidad con la API v1)
...
// ====================================================================
// FUNCIÓN 1: assignUserRole (compatibilidad v1)
// ====================================================================
exports.assignUserRole = functions.region(DEFAULT_FUNCTION_OPTIONS.region)
  .runWith({memory: DEFAULT_FUNCTION_OPTIONS.memory})
  .auth.user()
  .onCreate(async (userRecord) => {
    const user = userRecord;
    if (!user || !user.uid) {
      console.error(
        "Error: El objeto de usuario es nulo o indefinido, o falta el UID.",
      );
      return null;
    }

    console.log(`Cloud Function activada para el usuario: ${user.uid}`);
    console.log(`Correo electrónico del usuario: ${user.email || "N/A"}`);

    // Ignorar usuarios creados internamente por 'registrarEmpleado_v2'
    ...
    const userRecordFromAuth = await admin.auth().getUser(user.uid);
    if (userRecordFromAuth.customClaims && userRecordFromAuth.customClaims.role) {
      console.log(`El usuario ${user.uid} ya tiene el rol '${userRecordFromAuth.customClaims.role}'. La función assignUserRole no hará nada.`);
      return null;
    }
    ...
    try {
      ...
      console.log(`Asignando rol de '${userRole}' al usuario ${user.uid}.`);
      ...
      console.log(`Documento de Administrador creado para ${user.uid}.`);

      await admin.auth().setCustomUserClaims(user.uid, {role: userRole});
      console.log(`Custom Claim '{ role: "${userRole}" }' establecido para ${user.uid}.`);

      return null;
    } catch (error) {
      console.error(`Error en assignUserRole para el usuario ${user.uid}:`, error);
      return null;
    }
  });
...
exports.registrarEmpleado_v2 = functions.region(DEFAULT_FUNCTION_OPTIONS.region)
  .runWith({memory: DEFAULT_FUNCTION_OPTIONS.memory})
  .https.onCall(async (data, context) => {
    // data y context son la forma v1 de los callables
    const db = admin.firestore();

    // 1. Verificación de administrador
    if (!context || !context.auth || !context.auth.token || context.auth.token.role !== APP_CONSTANTS.ROLES.ADMIN) {
    ...
    try {
      ...
      const userRecord = await admin.auth().createUser({
        email: email,
        password: password,
        // Se crea con un nombre temporal para que el trigger `assignUserRole` lo ignore.
      ...
      // Actualizamos el perfil para poner el nombre correcto.
...
// ==================================================
exports.eliminarEmpleado_v2 = functions.region(DEFAULT_FUNCTION_OPTIONS.region)
  .runWith({memory: DEFAULT_FUNCTION_OPTIONS.memory})
  .https.onCall(async (data, context) => {
    const db = admin.firestore();
...
// ==================================================
exports.archiveOrders = functions.region(DEFAULT_FUNCTION_OPTIONS.region)
  .runWith({memory: DEFAULT_FUNCTION_OPTIONS.memory})
  .https.onCall(async (data, context) => {
    const db = admin.firestore();
    ...
    if (reportId) {
      ...
      if (!reportSnap.exists) {
        throw new functions.https.HttpsError("not-found", "El reporte seleccionado no existe.");
      }
...
// ==================================================
exports.eliminarHistorial_v2 = functions.region(DEFAULT_FUNCTION_OPTIONS.region)
  .runWith({memory: DEFAULT_FUNCTION_OPTIONS.memory})
  .https.onCall(async (data, context) => {
    const db = admin.firestore();
